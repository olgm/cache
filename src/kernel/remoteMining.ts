 ...
 */
function maxRemoteHarvesters(sourceCount: number): number {
 ...
 */
export function scanRemoteRoom(targetRoom: Room): RemoteSource[] {
  const ctrl = targetRoom.controller;
  if (ctrl && ctrl.owner) return []; // owned by anyone — see note above
  if (ctrl && ctrl.reservation) return []; // reserved by another player
  ...
  if (hostiles || keeperLairs) return [];
