 ...
 */
export function minerProductionFloor(energyCapacity: number, otherProducers: number): number {
...
function trySpawnRole(
  ...
  const minerFloor =
    role === "miner"
      ? minerProductionFloor(
          data.energyCapacity,
          roleCount(census, room.name, "miner") +
            roleCount(census, room.name, "harvester") +
            (reserved.miner || 0) +
            (reserved.harvester || 0),
        )
      : 0;
  const hasWeakMiners =
    role === "miner" && minerFloor > 0 && roomHasWeakMiners(room.name, data.rcl);
  const containerGap =
    role === "miner" &&
    minerFloor > 0 &&
  ...
  if (minerFloor > 0 && !recovering && !hasWeakMiners && !containerGap) {
    ...
    body = bodyForRole("miner", Math.min(data.energyCapacity, Math.max(budget, minerFloor)), data.rcl);
    if (data.energyAvailable < bodyCost(body)) return false; // wait for a proper miner
 ...
 */
function roomHasWeakMiners(home: string, rcl: number): boolean {
  if (rcl < 4) return false; // small miners are normal at low RCL
  let totalWork = 0;
  let count = 0;
  for (const name in Game.creeps) {
    const c = Game.creeps[name];
    if (c.memory.homeRoom !== home || c.memory.role !== "miner") continue;
    totalWork += c.getActiveBodyparts(WORK);
    count++;
  }
  if (count === 0) return false; // no miners at all — emergency paths handle this
