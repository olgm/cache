...
export function runMiner(creep: Creep): void {
  ...
  if (container) {
    if (!creep.pos.isEqualTo(container.pos)) travel(creep, container.pos, 0);
  } else if (!creep.pos.isNearTo(source)) {
    travel(creep, source, 1);
  }
...
function pickFreeSource(creep: Creep): Source | null {
  const data = getRoomData(creep.room);
  ...
  return (withContainer || free[0])?.source ?? null;
}
