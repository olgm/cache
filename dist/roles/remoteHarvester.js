...
// ---------------------------------------------------------------------------
function harvestRemote(creep, targetRoom) {
    // If not in the target room, travel there.
    if (creep.room.name !== targetRoom) {
        (0, movement_1.travel)(creep, new RoomPosition(25, 25, targetRoom), 20);
        return;
    }
...
// ---------------------------------------------------------------------------
function deliverHome(creep, home) {
    ...
    if (data.storage && data.storage.store.getFreeCapacity(RESOURCE_ENERGY) > 0) {
        if (creep.transfer(data.storage, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE)
