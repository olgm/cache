 ...
 */
function shouldRefillFromStorage(storageEnergy, spawnExtTowerFree) {
...
function buildReservedSet(me) {
    if (_reservedTick !== Game.time) {
        ...
        for (const name in Game.creeps) {
            ...
            if (c.name === me)
                continue;
            if (c.memory.role !== "hauler")
                continue;
            const tc = c.memory.targetContainer;
            if (tc)
                _reservedContainers.add(tc);
        }
    }
...
function collect(creep, data) {
    ...
    const anySourceEnergy = data.sources.some((s) => s.container && s.container.store[RESOURCE_ENERGY] > 0);
    if (!anySourceEnergy && data.storage) {
        const spawnExtTowerFree = [...data.spawns, ...data.extensions, ...data.towers].reduce((sum, s) => sum + s.store.getFreeCapacity(RESOURCE_ENERGY), 0);
...
function chooseSink(creep, data) {
    ...
    const spawnExt = spawnExtAll.filter((s) => s.store.getFreeCapacity(RESOURCE_ENERGY) > 0);
    const totalSpawnExtFree = spawnExt.reduce((sum, s) => sum + s.store.getFreeCapacity(RESOURCE_ENERGY), 0);
    ...
    if (data.storage &&
        ...
        if (cc) {
            ...
            const storageProgress = data.storage.store[RESOURCE_ENERGY] >= expansion_1.EXPANSION_STORAGE_RESERVE * 0.5;
