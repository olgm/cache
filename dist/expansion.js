...
exports.buildingExpansionReserve = buildingExpansionReserve;
 ...
 */
function buildingExpansionReserve(room) {
    if (ownedRoomCount() >= Game.gcl.level)
        return false;
    const ctrl = room.controller;
    if (!ctrl || !ctrl.my || ctrl.level < 4)
        return false;
    const storage = room.storage;
    if (!storage)
        return false;
    return storage.store[RESOURCE_ENERGY] < exports.EXPANSION_STORAGE_RESERVE;
}
// ---------------------------------------------------------------------------
// Memory
// ---------------------------------------------------------------------------
    ...
    switch (mem.state) {
        case "idle":
            ...
            if (canScout) {
                ...
                const haveAll = adj.length > 0 && adj.every((r) => mem.intel[r] && Game.time - mem.intel[r].lastSeen < INTEL_TTL);
                if (!haveAll)
                    mem.state = "scouting";
            }
            break;
